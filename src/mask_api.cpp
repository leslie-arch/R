 ...
 * cnts : the number of sequential bits
 */
void rleInit( RLE *R, siz h, siz w, siz m, uint *cnts ) {
  R->h=h;
  R->w=w;
  R->m=m;
  R->cnts=(m==0)?0:(uint *)malloc(sizeof(uint)*m);
  siz j;
  if(cnts)
    for(j=0; j<m; j++)
      R->cnts[j]=cnts[j];
...
char* rleToString( const RLE *R ) {
  ...
  for( i=0; i<m; i++ )
  {
    ...
    while( more ) {
      char c=x & 0x1f;
      x >>= 5;
      /* 这里的判断，非常巧妙，我想不出来这种方法  */
      more=(c & 0x10) ? x!=-1 : x!=0;
      if(more) c |= 0x20; /* 置最高位表示不是当前整数的最后一个字节  */
      c+=48;
      s[p++]=c;
    }
  }
...
void rleFrString( RLE *R, char *s, siz h, siz w ) {
  ...
  while( s[m] ) m++;
  cnts=(uint *)malloc(sizeof(uint)*m);
